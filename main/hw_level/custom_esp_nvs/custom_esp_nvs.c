...
bool custom_nvs_init(void){
    ...
    if(A_nvs_handle == NULL){
        ...
        for(enhe nvs_handle_name = NVS_HANDLE_STATUS; nvs_handle_name < NVS_NAMESPACE_MAX; nvs_handle_name++){
            #if CUSTOM_NVS_INIT_DEBUG
            printf("[%s] "COLOR_BLACK"[정보-INFO]\t %s custom_nvs_init() - NVS Handle Init : %s\n" COLOR_RESET, custom_getRuntimeString(), custom_esp_nvs_TAG, nvs_handle_to_string(nvs_handle_name));
            #endif
...
void custom_print_nvs_err(const esp_err_t input_err){
    ...
    case ESP_OK:
        ...
        printf("[%s] "COLOR_WHITE"[진행-OK]\t %s custom_print_nvs_err() - [ESP_OK] 작업 성공\n" COLOR_RESET,
               custom_getRuntimeString(), custom_esp_nvs_TAG);
        #endif
        break;
    ...
    default:
        ...
        printf("[%s] "COLOR_RED"[오류-ERROR]\t %s custom_print_nvs_err() - 알 수 없는 오류: 0x%X\n" COLOR_RESET,
               custom_getRuntimeString(), custom_esp_nvs_TAG, input_err);
        #endif
    }
}
...
bool custom_read_nvs(const enhe input_enhe_value, envs* input_envs_value){
    ...
    if (input_envs_value->c_nvs_key == NULL) {
        ...
        #if CUSTOM_READ_NVS_DEBUG
        printf("[%s] "COLOR_WHITE"[진행-OK]\t %s custom_read_nvs() - %s NVS에 %s 변수 존재 확인\n" COLOR_RESET, custom_getRuntimeString(), custom_esp_nvs_TAG, input_enhe_value == NVS_HANDLE_STATUS  ? "NVS_HANDLE_STATUS" 
        ...
        #endif
        #if CUSTOM_READ_NVS_DEBUG
        printf("[%s] "COLOR_WHITE"[진행-OK]\t %s custom_read_nvs() - TYPE : %s\n" COLOR_RESET, custom_getRuntimeString(), custom_esp_nvs_TAG, input_envs_value->envte_type == INT8 ? "INT8" 
        ...
        switch (input_envs_value->envte_type)
        {
            case INT8:
                ...
                #if CUSTOM_READ_NVS_DEBUG
                printf("[%s] "COLOR_WHITE"[진행-OK]\t %s custom_read_nvs() - VALUE : 0x%X\n" COLOR_RESET, custom_getRuntimeString(), custom_esp_nvs_TAG, input_envs_value->union_value.i8);
                #endif
                break;
            case UINT8: case BOOL:
                ...
                #if CUSTOM_READ_NVS_DEBUG
                printf("[%s] "COLOR_WHITE"[진행-OK]\t %s custom_read_nvs() - VALUE : 0x%X\n" COLOR_RESET, custom_getRuntimeString(), custom_esp_nvs_TAG, input_envs_value->union_value.u8);
                #endif
                break;
            case INT16:
                ...
                #if CUSTOM_READ_NVS_DEBUG
                printf("[%s] "COLOR_WHITE"[진행-OK]\t %s custom_read_nvs() - VALUE : 0x%X\n" COLOR_RESET, custom_getRuntimeString(), custom_esp_nvs_TAG, input_envs_value->union_value.i16);
                #endif
                break;
            case UINT16:
                ...
                #if CUSTOM_READ_NVS_DEBUG
                printf("[%s] "COLOR_WHITE"[진행-OK]\t %s custom_read_nvs() - VALUE : 0x%X\n" COLOR_RESET, custom_getRuntimeString(), custom_esp_nvs_TAG, input_envs_value->union_value.u16);
                #endif
                break;
            case INT32:
                ...
                #if CUSTOM_READ_NVS_DEBUG
                printf("[%s] "COLOR_WHITE"[진행-OK]\t %s custom_read_nvs() - VALUE : 0x%lX\n" COLOR_RESET, custom_getRuntimeString(), custom_esp_nvs_TAG, input_envs_value->union_value.i32);
                #endif
                break;
            case UINT32:
                ...
                #if CUSTOM_READ_NVS_DEBUG
                printf("[%s] "COLOR_WHITE"[진행-OK]\t %s custom_read_nvs() - VALUE : 0x%lX\n" COLOR_RESET, custom_getRuntimeString(), custom_esp_nvs_TAG, input_envs_value->union_value.u32);
                #endif
                break;
            case INT64:
                ...
                #if CUSTOM_READ_NVS_DEBUG
                printf("[%s] "COLOR_WHITE"[진행-OK]\t %s custom_read_nvs() - VALUE : 0x%llX\n" COLOR_RESET, custom_getRuntimeString(), custom_esp_nvs_TAG, input_envs_value->union_value.i64);
                #endif
                break;
            case UINT64:
                ...
                #if CUSTOM_READ_NVS_DEBUG
                printf("[%s] "COLOR_WHITE"[진행-OK]\t %s custom_read_nvs() - VALUE : 0x%llX\n" COLOR_RESET, custom_getRuntimeString(), custom_esp_nvs_TAG, input_envs_value->union_value.u64);
                #endif
                break;
            case STRING:
                input_envs_value->union_value.str.str_data = NULL;
...
bool custom_write_nvs(enhe input_enhe_value, envs* input_envs_value){
    ...
    if (input_envs_value->c_nvs_key == NULL) {
        ...
        #if CUSTOM_WRITE_NVS_DEBUG
        printf("[%s] "COLOR_WHITE"[진행-OK]\t %s custom_write_nvs() - input_envs_value->c_nvs_key : %s\n" COLOR_RESET, custom_getRuntimeString(), custom_esp_nvs_TAG, input_envs_value->c_nvs_key);
...
void custom_commit_nvs(const enhe input_enhe_value){
    ...
    printf("[%s] "COLOR_WHITE"[진행-OK]\t %s custom_commit_nvs() - %s Commit\n" COLOR_RESET, custom_getRuntimeString(), custom_esp_nvs_TAG, nvs_handle_to_string(input_enhe_value));
    #endif
